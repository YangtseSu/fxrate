...
- Historical `--date` convert: a covered date reuses the cache with no network; with no local history an uncovered date tries a sync then exits 1; a date outside the covered range skips the sync — a future one falls back to the latest local day, one before the history exits 1 (`-u` still syncs); a weekend/holiday date falls back to the previous business day (noted on stderr, rate date shown is that day); ECB EUR-based cross math matches `chart`
